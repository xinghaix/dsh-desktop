...
| `HostSidecar.OnUnexpectedExit` | Mid-run Host exit → host-error (`crash`) |
...
| G | Host crash / exit mid-run → friendly page | **PASS** | `G-host-crash.log` / `.png` — READY then `exit 1`; kind `crash`; “Host exited unexpectedly” |
...
# G crash: Host prints DSH_HOST_READY then exit 1
