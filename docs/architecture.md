...
推荐入口：`start:wails` / `dev:wails` / `smoke:wails` / `start:host`。LEGACY `NativeWindow` / Tray / `main.ts` **不是**当前主产品路径，但仍保留为最后手段 fallback（需显式允许；见 `dsh-plugin-desktop/docs/legacy-shell-fallback.md`）。在发布切换完成前，legacy-builder 仍是默认产品 CI 打包路径。
...
1. Wails 壳启动并拉起 Cordis Host sidecar（优先 Node host-main；需要时 LEGACY-as-Node；显式允许时 QUARANTINED LEGACY main.ts）。
...
- **Native runtime**：主路径为 Wails 窗口/托盘/对话框；LEGACY 适配器保留为最后手段。`desktopRuntime` 只供 Desktop 自有 row 使用。
...
Desktop 级确认、警告、错误与结果不会进入 Web Client 组件树。`DesktopDialogWindow` 会创建独立、沙箱化的模态 `NativeWindow`，应用共享的空白 utility frame，并在可能时以当前 generation 窗口为 parent，只接受一次有界本地结果。恢复模式与新增 Profile 是使用同一套无标题 frame 的独立 Desktop-owned 窗口。恢复页面本身使用 shadcn，先展示原因，再提供四个工作流 Tab；破坏性恢复操作会把确认交回 `DesktopDialogWindow`。

### 原生 Shell generation 与平台 adapter（LEGACY 最后手段路径）

`LEGACYRuntime` 负责协调 Host 与原生桌面环境，但不直接拥有窗口和托盘的细节。每次启动由一个 `LEGACYShellGeneration` module 完整拥有 `NativeWindow`、`Tray`、相关 LEGACY listener、导航限制、外链处理和缩放快捷键。释放 generation 必须通过其幂等 `release()` interface 完成，调用方不能跨 generation 缓存或单独销毁这些资源。

平台差异集中在启动时选择一次的 `LEGACYPlatformStrategy` seam。Windows、macOS 与 Linux adapter 声明目录选择、Shell 模式切换和更新下载能力，并负责各自的菜单、Dock 图标与原生材质操作。新的平台分支应进入对应 adapter；generation 与 runtime 中只保留各平台共享的生命周期流程。
...
Launcher 私有的 `desktopRuntime`、`desktopPnpmBootstrap`、LEGACY executable、Node helper 和 ABI 环境不是第三方 API。稳定包的公开 contract 是 `dsh-plugin-desktop/profile-service` 与 `dsh-plugin-desktop/pnpm`；Beta 包提供对应的 `dsh-plugin-desktop-beta/*` 路径。

## 打包与运行时闭包

当前产品 CI 默认仍使用 LEGACY Builder 和 `app.asar`（Wails `package:wails` / AppImage 路径并行演进；发布切换前以 legacy-builder 为准），但需要物理 unpack 的依赖（例如 pnpm、node-pty、Windows ACL/native 文件）会放在 `app.asar.unpacked`。Packaged runtime gate 会检查 ASAR 入口和物理运行时入口，profile fallback 不能把符号链接指向无法被 Node 解析的虚拟 ASAR 路径。
